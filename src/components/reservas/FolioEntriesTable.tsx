// src/components/reservas/FolioEntriesTable.tsx
"use client";
    ...
    <div className="surface-2">
      ...
      <div className="rounded-2xl border border-subtle overflow-hidden">
        ...
        <div className="grid grid-cols-12 px-3 py-2 text-xs font-semibold opacity-70 bg-black/5 dark:bg-white/5">
          ...
          <div className="col-span-6 md:col-span-2 text-right">VALOR</div>
          ...
          {entries.map((e) => {
            ...
            return (
              ...
              >
                ...
                <div className="hidden md:block md:col-span-1 text-right tabular-nums">{qty}</div>
                <div className="col-span-6 md:col-span-2 text-right font-medium whitespace-nowrap tabular-nums pr-4">
                  {formatCurrency(total)}
                </div>
                <div className="col-span-6 md:col-span-1 flex justify-end gap-1 shrink-0 w-[96px]">
                  <IconBtn title="Ver detalhes" onClick={() => onView(e)}><Search className="h-4 w-4" /></IconBtn>
                  <IconBtn title="Editar lançamento" onClick={() => onEdit(e)}><Pencil className="h-4 w-4" /></IconBtn>
                  <IconBtn title="Excluir lançamento" onClick={() => onDelete(e.id)}><Trash2 className="h-4 w-4" /></IconBtn>
                </div>
              </div>
            );
          })}
