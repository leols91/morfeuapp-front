...
export function EditFolioEntryModal({ open, onClose, entry, onConfirm, onDelete }: Props) {
  ...
  return (
    <>
      {/* ===== OVERLAY ÚNICO (resolve a “barra sem blur”) ===== */}
      <div
      ...
      >
        {/* camada de blur/cor cobrindo 100% */}
        ...
        {/* contêiner relativo do conteúdo */}
        <div className="relative min-h-full grid place-items-center p-4 overflow-y-auto pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
          <div
            onClick={stop} // impede fechar ao clicar dentro
      ...
      {/* ===== CONFIRM MODAL: mesmo padrão de overlay único ===== */}
      {confirmOpen && (
        <div className="fixed inset-0 z-[10001]" onClick={() => setConfirmOpen(false)} role="dialog" aria-modal="true">
          <div className="absolute inset-0 bg-black/50 supports-[backdrop-filter]:bg-black/30 supports-[backdrop-filter]:backdrop-blur-[2px]" />
          <div className="relative min-h-full grid place-items-center p-4">
            <div
              onClick={(e) => e.stopPropagation()}
              className="w-full max-w-sm rounded-2xl border border-subtle bg-white dark:bg-[#0F172A] shadow-soft"
            >
              <div className="px-5 py-4 border-b border-subtle">
                <h4 className="text-base font-semibold">Excluir lançamento</h4>
              </div>
              <div className="px-5 py-5 space-y-2 text-sm">
                <p className="opacity-80">Tem certeza que deseja excluir este lançamento? Essa ação não pode ser desfeita.</p>
              ...
              <div className="px-5 py-4 border-t border-subtle flex justify-end gap-2">
                ...
                <Button className="bg-red-600 text-white hover:bg-red-700" onClick={confirmDelete} disabled={deleting}>
