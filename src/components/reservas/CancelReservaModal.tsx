...
const schema = z.object({
  ...
  penalty: z.union([z.number(), z.nan()]).optional(), // RHF envia string -> tratamos abaixo
...
export function CancelReservaModal({ open, onClose, onConfirm, hospede, periodo, acomodacao }: Props) {
  ...
  async function submit(v: FormData) {
    ...
    try {
      const payload: CancelReservaPayload = {
        reason: v.reason,
        penalty: Number.isNaN(v.penalty as any) ? null : (typeof v.penalty === "number" ? v.penalty : Number(v.penalty)),
        notes: v.notes || null,
      };
          ...
          <div className="px-4 py-4 md:px-6 md:py-6">
            <form id="form-cancel" onSubmit={form.handleSubmit(submit)} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <label className="text-xs opacity-70 block">Motivo</label>
                  <select
                ...
                <div>
                  <label className="text-xs opacity-70 block">Taxa/penalidade (R$) — opcional</label>
                  <input
                    type="number"
                    step="0.01"
                    inputMode="decimal"
                    className="mt-1 h-9 w-full rounded-2xl border-subtle bg-transparent px-3"
                    {...form.register("penalty", { setValueAs: (v) => (v === "" ? Number.NaN : Number(v)) })}
                  />
                </div>
              </div>
